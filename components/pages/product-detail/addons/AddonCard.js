...
export default function AddonCard({ option, addonGroup, isSelected, onSelect, onExpand }) {
  ...
  return (
    <div
      ...
      className={`relative min-w-[180px] cursor-pointer rounded-2xl border-2 p-2 transition-all ${
    ...
    >
      ...
      <button
        ...
        className="absolute top-4 left-4 z-10 rounded-2xl bg-white p-1 shadow transition-colors hover:bg-gray-100"
      >
        <svg
      ...
      {/* Name */}
      <h4 className="mb-2 text-center text-sm font-medium text-gray-900">{option.name}</h4>

      {/* Price */}
      <div className="flex items-center justify-center gap-2">
        {option.salePrice > 0 && option.salePrice < option.price ? (
          <>
            <span className="font-bold text-red-600">+AED {option.salePrice}</span>
            <span className="text-xs text-gray-400 line-through">AED {option.price}</span>
          </>
        ) : option.price > 0 ? (
