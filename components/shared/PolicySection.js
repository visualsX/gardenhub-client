...
export default function PolicySection({ icon, title, children }) {
  return (
    <div className="group hover:border-primary rounded-3xl border border-green-100 bg-white p-8 shadow-sm transition-all hover:shadow-lg">
      <div className="mb-6 flex items-center gap-4">
        <div className="bg-primary flex h-14 w-14 items-center justify-center rounded-2xl text-white shadow-lg transition-transform group-hover:scale-110 group-hover:rotate-6">
          {icon}
        </div>
    ...
    <div className="space-y-3">
      {steps.map((step, index) => (
        <div key={index} className="flex items-start gap-3 rounded-lg bg-green-50 p-3">
          <div className="bg-primary flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold text-white">
            {index + 1}
          </div>
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      {columns.map((column, colIndex) => (
        <div
          key={colIndex}
          className={`rounded-2xl p-5 ${column.highlight ? 'bg-green-50' : 'bg-gray-50'}`}
        >
          ...
          <div className="space-y-2">
            {column.items.map((item, itemIndex) => (
              <div key={itemIndex} className="flex items-start gap-2">
                <div
                  className={`mt-1 h-1.5 w-1.5 shrink-0 rounded-full ${column.highlight ? 'bg-primary' : 'bg-gray-400'}`}
                ></div>
