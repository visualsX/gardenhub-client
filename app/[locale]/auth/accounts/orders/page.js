...
export default async function OrdersPage({ searchParams }) {
  ...
  return (
    <div className="space-y-8 pb-10">
          ...
          {TABS.map((tab) => {
            ...
            return (
              <Link
                ...
                className={`group relative flex items-center gap-2 rounded-full px-5 py-2 text-sm font-bold whitespace-nowrap transition-all duration-300 ${
                  isActive
      ...
      {orders.length === 0 ? (
        <div className="flex min-h-[400px] flex-col items-center justify-center space-y-6 rounded-3xl border border-gray-100 bg-white p-8 text-center shadow-sm ring-1 ring-black/5">
          ...
          <div className="max-w-xs space-y-2">
            ...
            <p className="text-sm text-gray-500">
              Looks like you haven't placed any {activeTab !== 'all' ? activeTab.toLowerCase() : ''}{' '}
              orders.
            </p>
          </div>
          <Link
            href="/shop"
            className="bg-primary rounded-full px-6 py-3 text-sm font-bold text-white shadow-md transition-transform hover:scale-105 active:scale-95"
          >
            Explore Shop
          </Link>
        </div>
