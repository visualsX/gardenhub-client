...
// Helper to clean up payment method display
const formatPaymentMethod = (method) => {
    if (!method) return 'N/A';
    // Remove domain prefix if exists (e.g., Planta.Domain.Entities.PaymentMethod -> PaymentMethod)
    const clean = method.includes('.') ? method.split('.').pop() : method;
    // Handle camelCase or SCREAMING_SNAKE_CASE if necessary
    return clean.replace(/_/g, ' ').replace(/([A-Z])/g, ' $1').trim().toUpperCase();
};
...
export default async function OrderDetailPage({ params }) {
    ...
    const statusStyle = STATUS_CONFIG[order.status] || { color: 'text-gray-700 bg-gray-50 ring-gray-600/10', dot: 'bg-gray-500' };
        ...
        <div className="mx-auto max-w-5xl space-y-8 pb-20">
            ...
            <div className="flex flex-col gap-6">
                <Link
                    ...
                    className="flex w-fit items-center gap-2 text-sm font-bold text-gray-500 transition-colors hover:text-primary"
                >
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 border-b border-gray-100 pb-8">
                    <div className="space-y-1">
                        <div className="flex items-center gap-3">
                            ...
                            <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-wider ring-1 ring-inset ${statusStyle.color}`}>
            ...
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
                ...
                <div className="lg:col-span-2 space-y-8">
                    <div className="overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-sm ring-1 ring-black/5">
                        <div className="border-b border-gray-50 bg-gray-50/50 px-6 py-4">
                            <h3 className="text-xs font-black uppercase tracking-widest text-gray-500">Order Contents</h3>
                        </div>
                        <div className="divide-y divide-gray-50">
                            {order.items.map((item, idx) => (
                                <div key={idx} className="flex gap-6 p-6 transition-colors hover:bg-gray-50/50">
                                    <div className="relative h-24 w-24 shrink-0 overflow-hidden rounded-2xl bg-gray-50 ring-1 ring-gray-100">
                                        <img
                                    ...
                                    <div className="flex flex-1 flex-col justify-between py-1">
                                        <div className="space-y-4">
                                            <div className="flex items-start justify-between">
                                                ...
                                                <p className="font-bold text-gray-900 tabular-nums">{formatCurrency(item.itemTotal)}</p>
                                            </div>

                                            {/* Addons Section */}
                                            {item.addons?.length > 0 && (
                                                <div className="mt-2 space-y-3 rounded-2xl bg-gray-50/50 p-4 ring-1 ring-gray-100/50">
                                                    <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Included Addons</p>
                                                    <div className="space-y-3">
                                                        {item.addons.map((addon) => (
                                                            <div key={addon.id} className="flex items-center gap-3">
                                                                <div className="h-10 w-10 shrink-0 overflow-hidden rounded-lg bg-white ring-1 ring-gray-100">
                                                                    <img
                                                                ...
                                                                <div className="flex flex-1 items-center justify-between">
                                                                    <div>
                                                                        ...
                                                                        <p className="text-[10px] font-medium text-gray-400 capitalize">{addon.optionName}</p>
                                                                    </div>
                                                                    <div className="text-right">
                                                                        <p className="text-[10px] font-bold text-gray-900 tabular-nums">+{formatCurrency(addon.priceAtPurchase)}</p>
                                                                        <p className="text-[9px] font-medium text-gray-400 uppercase tracking-tight">Qty: {addon.quantity}</p>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        ))}
                                        ...
                                        <div className="mt-4 flex items-center justify-between text-xs">
                                            <p className="font-bold text-gray-400">
                                                Quantity <span className="text-gray-900 ml-1">{item.quantity}</span>
                                            </p>
                    ...
                    {order.trackingHistory?.length > 0 && (
                        <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm ring-1 ring-black/5">
                            <h3 className="mb-6 text-xs font-black uppercase tracking-widest text-gray-500">Logistics Timeline</h3>
                            <div className="space-y-6">
                                {order.trackingHistory.map((step, idx) => (
                                    <div key={idx} className="relative flex gap-4">
                                        {idx !== order.trackingHistory.length - 1 && (
                                            <div className="absolute left-2.5 top-6 h-full w-px bg-gray-100"></div>
                                        )}
                                        <div className={`mt-1 h-5 w-5 shrink-0 rounded-full border-4 border-white ring-2 bg-primary ring-primary/20`}></div>
                                        <div className="space-y-1 pb-4">
                                            <p className="text-sm font-bold text-gray-900">{step.status}</p>
                                            <p className="text-xs font-medium text-gray-500">{step.notes || 'No notes provided'}</p>
                                            <p className="text-[10px] font-bold text-gray-300 uppercase tracking-widest">{formatDate(step.createdAt)}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                <div className="space-y-8">
                    {/* Totals Card */}
                    <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm ring-1 ring-black/5">
                        <h3 className="mb-6 text-xs font-black uppercase tracking-widest text-gray-500">Order Ledger</h3>
                        <div className="space-y-4">
                    ...
                    {/* Shipping Address */}
                    <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm ring-1 ring-black/5">
                        <h3 className="mb-4 text-xs font-black uppercase tracking-widest text-gray-500">Shipping Destination</h3>
                    ...
                    {order.transactions?.length > 0 && (
                        <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm ring-1 ring-black/5">
                            <h3 className="mb-4 text-xs font-black uppercase tracking-widest text-gray-500">Transaction Info</h3>
                            <div className="space-y-4">
                                {order.transactions.map((tx, idx) => (
                                    <div key={idx} className="flex flex-col gap-2 rounded-2xl bg-gray-50/50 p-4 ring-1 ring-gray-100/50">
                                        <div className="flex flex-col gap-1.5">
                                            ...
                                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{formatDate(tx.createdAt)}</p>
                                        </div>
                                        {tx.amount > 0 && (
                                            <div className="mt-1 flex items-center justify-between border-t border-gray-100/50 pt-2 text-[10px] font-bold uppercase tracking-widest">
