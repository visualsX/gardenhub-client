...
export const useVariantSelection = (product) => {
    ...
    const [selectedOptions, setSelectedOptions] = useState(() => {
        ...
        if (!hasUrlOptions && product?.variants?.length > 0) {
            ...
            const inStockVariant = product.variants.find(v => v.stockQuantity > 0 && v.isAvailable !== false);
    ...
    const updateUrl = useCallback((newOptions) => {
    ...
    const isOptionDisabled = useCallback((optionName, value) => {
    ...
    const currentPrice = selectedVariant
        ? (selectedVariant.salePrice > 0 ? selectedVariant.salePrice : variantPrice(selectedVariant))
    ...
    const canAddToCart = allOptionsSelected && isAvailable && quantity > 0 && quantity <= availableStock;
